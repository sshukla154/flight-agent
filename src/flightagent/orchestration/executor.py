"""Bounded-concurrency task execution with retry (T24 + T25).
...
from flightagent.config.models import FlightAgentSettings, RetrySettings
    ...
    *,
    ...
    with task_context(task.task_id):
        return await _search_and_record(
            task, provider, retry_settings=retry_settings, retry_budget=retry_budget
        )
